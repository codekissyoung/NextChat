...
const cn = {
  ...
  Store: {
    ...
    Prompt: {
      ...
      Topic:
        "根据对话内容生成一个简洁的标题（4-8个字）。要求：1. 直接返回标题，不要任何解释或多余文本；2. 不要使用标点符号；3. 准确概括对话核心内容。示例格式：'NextChat部署'、'ReACT实现'、'工具调试'。⚠️ 重要：只有在对话内容确实无法概括、过于简短或随意闲聊时才返回'闲聊'，否则必须根据实际内容生成有意义的标题。",
      Summarize:
